...
def main():
    with sync_playwright() as p:
        ...
        for tender in tenders:
            ...
            messages = [
                SystemMessage(
                content="Ты помошник по тендерам. Я буду искать тендеры по ключевым словам и присылать тебе. Твоя задача определить подходит ли тендер тебе или нет. Если не подходит пиши 'Не подходит'. Если подходит напиши почему ты так думаешь. Мы поставляем сырье (не производные, не готовые продукты), а именно Ксантоновая камедь, полиэфирное волокно, гуаровая камедь, диоксид титана, пальмовое масло, кокосовое масло, защищённый жир, заменитель какао масла. Проанализируй заголовок тендера и реши может ли это быть потенциально наш клиент"
                ),
                HumanMessage(
